...
// YOLO26-n with late (decision-level) RGB-IR fusion. Two complete detectors
// run side by side, one per modality; Weighted Box Fusion merges the two box
// lists. Each detector is generated by one function per modality; the only
// hand-set numbers are the branch separation and the skip lanes.
